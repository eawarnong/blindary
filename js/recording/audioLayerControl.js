function audioLayerControl(elementContext)
{
    ...
    this.save = function save()
    {
        ...
        this.toWave().toBlobUrlAsync("application/octet-stream");
        // saveLink.href = url;
    ...
    this.createDropHandler = function createDropHandler()
    {
        ...
        {
            console.log('createDropHandler onFinish');
            activeLayer(this.resultArrayBuffer);
            // console.log('length ' + this.resultArrayBuffer.byteLength);
    ...
    { 
        ...
        if(window.location.pathname == '/recordSound') {
            $('#txt_record').html(convertMilliToTime(Math.round(duration)*1000));
            setVideoTime(Math.round(duration));
        }
...
function activeLayer(resultArrayBuffer) {
    console.log('activeLayer');
    var eventHost = filedb.eventHost;
    // $('#app-progress')[0].style['width'] = '50%';
    activeAudioLayerControl = eventHost.elementContext;
    eventHost.audioPlayback.audioContext.decodeAudioData(resultArrayBuffer, eventHost.decodeAudioFinished, eventHost.decodeAudioFailed);
};
